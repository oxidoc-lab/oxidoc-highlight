...
mod tests {
    ...
    fn hl(code: &str) -> String { render(code, &MarkdownScanner.scan(code)) }

    #[test]
    fn heading() { assert!(hl("# Hello").contains("tok-keyword")); }
    #[test]
    fn code_span() { assert!(hl("`code`").contains("tok-string")); }
    #[test]
    fn bold() { assert!(hl("**bold**").contains("tok-keyword")); }
    #[test]
    fn link() { assert!(hl("[text](url)").contains("tok-string")); }
    #[test]
    fn code_fence() { assert!(hl("```rust\nlet x = 1;\n```").contains("tok-string")); }
}
