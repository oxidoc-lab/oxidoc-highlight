...
const KEYWORDS: &[&[u8]] = &[
    b"break", b"case", b"chan", b"const", b"continue", b"default", b"defer",
    b"else", b"fallthrough", b"for", b"func", b"go", b"goto", b"if", b"import",
    b"interface", b"map", b"package", b"range", b"return", b"select", b"struct",
    b"switch", b"type", b"var", b"true", b"false", b"nil",
];

const TYPES: &[&[u8]] = &[
    b"bool", b"byte", b"complex64", b"complex128", b"error", b"float32", b"float64",
    b"int", b"int8", b"int16", b"int32", b"int64", b"rune", b"string",
    b"uint", b"uint8", b"uint16", b"uint32", b"uint64", b"uintptr", b"any",
];

const BUILTINS: &[&[u8]] = &[
    b"append", b"cap", b"close", b"complex", b"copy", b"delete", b"imag",
    b"len", b"make", b"new", b"panic", b"print", b"println", b"real", b"recover",
];
...
impl Scanner for GoScanner {
    fn scan(&self, code: &str) -> Vec<Token> {
        ...
        while i < b.len() {
            ...
            if c == b'`' { if let Some(end) = scan_backtick_string(b, i) { tokens.push(Token { kind: TokenKind::String, start: i, end }); prev_kind = Some(TokenKind::String); i = end; continue; } }
            ...
            if let Some((end, ident)) = scan_ident(b, i) {
                let kind = if is_keyword(ident, KEYWORDS) { TokenKind::Keyword }
                    ...
                    else if is_keyword(ident, BUILTINS) { TokenKind::Builtin }
...
mod tests {
    ...
    fn hl(code: &str) -> String { render(code, &GoScanner.scan(code)) }

    #[test]
    fn func() { assert!(hl("func main() {}").contains("tok-keyword")); }
    #[test]
    fn types() { assert!(hl("var x int").contains("tok-type")); }
    #[test]
    fn builtin() { assert!(hl("len(s)").contains("tok-builtin")); }
    #[test]
