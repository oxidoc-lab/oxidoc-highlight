...
mod tests {
    ...
    fn hl(code: &str) -> String {
        render(code, &SqlScanner.scan(code))
    }

    #[test]
    fn select() {
        assert!(hl("SELECT * FROM users").contains("tok-keyword"));
    }
    #[test]
    fn string() {
        assert!(hl("WHERE name = 'foo'").contains("tok-string"));
    }
    #[test]
    fn comment() {
    ...
    #[test]
    fn number() {
        assert!(hl("LIMIT 10").contains("tok-number"));
    }
    #[test]
    fn builtin() {
        assert!(hl("COUNT(*)").contains("tok-builtin"));
    }
}
