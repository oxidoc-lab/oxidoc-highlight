...
mod tests {
    ...
    fn hl(code: &str) -> String {
        render(code, &JavaScanner.scan(code))
    }

    #[test]
    fn annotation() {
        assert!(hl("@Override").contains("tok-attr"));
    }
    #[test]
    fn keyword() {
        assert!(hl("public class Foo").contains("tok-keyword"));
    }
    #[test]
    fn type_name() {
        assert!(hl("public class Foo").contains("tok-type"));
    }
    #[test]
    fn method() {
        assert!(hl("foo.bar()").contains("tok-function"));
    }
    #[test]
    fn string() {
