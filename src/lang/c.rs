    ...
    fn scan(&self, code: &str) -> Vec<Token> {
        ...
        while i < b.len() {
            ...
            if let Some((end, ident)) = scan_ident(b, i) {
                let kind = if is_keyword(ident, C_KEYWORDS)
                    || (self.cpp && is_keyword(ident, CPP_EXTRA))
                {
                    TokenKind::Keyword
...
mod tests {
    ...
    fn hl(code: &str) -> String {
        render(code, &CScanner { cpp: false }.scan(code))
    }
    fn hl_cpp(code: &str) -> String {
        render(code, &CScanner { cpp: true }.scan(code))
    }

    #[test]
    fn preprocessor() {
        assert!(hl("#include <stdio.h>").contains("tok-attr"));
    }
    #[test]
    fn keyword() {
        assert!(hl("if (x) return 0;").contains("tok-keyword"));
    }
    #[test]
    fn types() {
        assert!(hl("int x;").contains("tok-type"));
    }
    #[test]
    fn function() {
        assert!(hl("printf(\"hi\")").contains("tok-function"));
    }
    #[test]
    fn cpp_class() {
        assert!(hl_cpp("class Foo {}").contains("tok-keyword"));
    }
}
